...
$encdata  = App_Encrypt::docencrypt($data,$keyver);
$encdata2 = App_Encrypt::docencrypt($data2,$keyver);
	$params = array('MOBILEID'=>$mobileid,'KEYVER'=>$keyver,'NUMBER'=>$number,'VERSION'=>$version,'DATA'=>$encdata['data'],'DATA2'=>$encdata2['data']);
    ...
    if ($result['status'] === 'success') {
      $id =  $result['data'];	
$path = '/var/www/html/incash/u/data/'.$id.'/';
if(!is_dir($path)){
	mkdir($path, 0777, true);
}
      file_put_contents("/var/www/html/incash/u/data/$id/front.key",$encdata['key']);
      file_put_contents("/var/www/html/incash/u/data/$id/back.key",$encdata2['key']);
	  file_put_contents("/var/www/html/incash/u/data/$id/front",$encdata['data']); 
      file_put_contents("/var/www/html/incash/u/data/$id/back",$encdata2['data']);
      return $id;
    } else {
      return 0;
    }
  ...
  public function updateDocument($docid,$mobileid,$version,$keyver,$data,$data2,$number,$expires,$same,$same2) {
...
$encdata2 = ""; 
	...
	$uinfof = $uinfo->update(
                                        ...
                                        $uinfo->getAdapter()->quoteInto('doc_info_id = ?', $docid)
                                );
    if ($result['status'] === 'success') {
...
if(!is_dir($path)){
	mkdir($path, 0777, true);
				...
				if ($same === "FALSE" ) {
					/*$m = new App_Messenger();
					error_log('mail send1111');
				$mess="<pre>".print_r($encdata ,true)."=".$docid."=".$same."=".$same2;
				$m->sendMessage("Message:".$mess,"incashmeapp@gmail.com",'1');*/
					 file_put_contents("/var/www/html/incash/u/data/$docid/front.key",$encdata['key']); 
					 file_put_contents("/var/www/html/incash/u/data/$docid/front",$encdata['data']); 
                }
                if ($same2 === "FALSE") {
					 file_put_contents("/var/www/html/incash/u/data/$docid/back.key",$encdata2['key']);
					 file_put_contents("/var/www/html/incash/u/data/$docid/back",$encdata2['data']);
                }
	
    /*if ($data != "") {
     file_put_contents("/var/www/html/incash/u/data/$docid/front.key",$encdata); 
    }

    if ($data2 != "") {
      file_put_contents("/var/www/html/incash/u/data/$docid/back.key",$encdata2);
    }*/

...
class App_Event_Mobws_CellphoneController_emailuserstatement extends App_Event_WsEventAbstract  {
	...
	public function execute(&$session_data,&$pview,&$cthis){
                ...
                $pview->datefrom_email = date("d_M_Y", strtotime($getdatefrom));
                $pview->dateto_email = date("d_M_Y", strtotime($getdateto));
...
$pdf->fillItems1($debit_card_funding,$credit_card_funding,$total_money_send,$total_money_recieved,$bank_funding, $bank_withdraw,$impc_redeemed,$total_donation,$scan_buy,$scan_pay,$scan_gift,$scan_donate);
$pdf->fillItems2($uid,$from_date, $to_date,$total);
$pdf->Image($imagePath1,10,10,0,0);
$pdf->Image($imagePath2,75,10,0,0);
$pdf->Image($imagePath3,140,10,0,0); 

$pdf->FillHeadInfo($info,$result['result']['data']['datefrom'],$result['result']['data']['dateto']); 
$pdf->FillItems($items);
$path = '/var/www/html/incash/u/statement/';
$user_folder = $uid;
$filepath = $path.$user_folder;
if(!file_exists($filepath)){
	mkdir($filepath, 0777);
}

$filename = 'statement_'.$pview->datefrom_email.'_to_'. $pview->dateto_email.'.pdf';
...
//mail code start
				$to = $email;
				//$user_name = 'sahejin mansuri';
				$user_name = $user->getFirstName()." ".$user->getLastName();
				$message = 'Hi '. $fname." ". $lname."<br><br>";
				$message .= 'InCashMe&trade; : Your friend '.$user_name.' has send you Statement Detail<br><br>';
				...
				if(file_exists($filepath.'/'.$filename)){
					$data = chunk_split(base64_encode(file_get_contents($filepath.'/'.$filename)));
				}else{
					$data = "";
				}
				$data2 = '';
				$fileName2 = '';
				$m = new App_Messenger();
				$m->sendMessageAttachment($message,$to,'InCashMe : Statement',$fileType,$data,$data2,$fileName1,$fileName2);

				$result = array();
				$dataRes=array('title'=>'Success','message'=>'You have successfully send statement to '.$fname.' '.$lname.'. Thank you.');
                $result['result']['status'] = 'success';
                $result['result']['value'] = '';

...
class App_Event_Mobws_CellphoneController_donate extends App_Event_WsEventAbstract {
    ...
    public function execute(){
		...
		//get virtual pos id
                ...
                if (!$to_id) {
                  $m = new App_Messenger();
                  $c_to->sendMessage("InCashMe™: " . $u_from->getFirstName() . " has tried to send you INR $amount. Please sign up at incashme.in",'InCashMe™: Cellphone Registration');
                ...
                if ($u_to->getStatus() === "unconfirmed") {
                  $c_to->sendMessage("InCashMe™: " . $u_from->getFirstName() . " has tried to send you INR $amount, but your email is not currently activated. Please check your email and click 'Verify Now' to active your account.", 'InCashMe: Account Activate');
                ...
                $c_from->sendMessage("InCashMe™: You have sent INR $amount to $to", 'InCashMe™: Money Sent');
                $c_to->sendMessage("InCashMe™: You have received INR $amount from " . $c_from->getCellphone() . " Message: $message", 'InCashMe™: Money Recieved');

...
class App_Event_Mw_ProfileController_deleteaccount extends App_Event_WsEventAbstract  {
	...
	public function execute(&$session_data,&$pview,&$cthis){
		...
		if ($this->_request->getParam('doaction') != null) {
			...
			if ($user->passwordMatches($this->_request->getParam('PASSWORD')) && !($user->getBalance() > 0) && !($user->getNoActiveCodes() > 0)) {
				...
				$m = new App_Messenger();
				$m->sendMessage("Your account has been successfully deleted.",$user->getEmail(),'1''InCashMe: Account Deleted');

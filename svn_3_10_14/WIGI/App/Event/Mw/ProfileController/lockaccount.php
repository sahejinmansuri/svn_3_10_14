...
class App_Event_Mw_ProfileController_lockaccount extends App_Event_WsEventAbstract  {
	...
	public function execute(&$session_data,&$pview,&$cthis){
		...
		if ($this->_request->getParam('doaction') != null) {
			...
			if ($user->passwordMatches($this->_request->getParam('PASSWORD'))) {
				...
				$businessid = $user->getMerchantId();
				
				$m = new App_Messenger();
                $m->sendMessage("Your account \"$businessname\" ($businessid) has been successfully locked.",$user->getEmail(),'1','InCashMe : Lock Account');

...
class App_Event_Mw_ProfileController_unlockcell extends App_Event_WsEventAbstract  {
		...
		if ($item != null && is_numeric($item)) {
			...
			if ($this->_request->getParam('doaction') != null) {
				...
				if ($user->passwordMatches($password) && $c->pinMatches($pin)) {
					...
					$m = new App_Messenger();
					$m->sendMessage("Your cell phone, $cellphone, has been successfully unlocked on your account.",$user->getEmail(),'1','InCashMe : Unlock Cellphone');

...
class App_Event_Cw_DashboardController_savequestions extends App_Event_WsEventAbstract  {
	...
	public function execute(&$session_data,&$pview,&$cthis){
				...
				for($i=0;$i<3;$i++){
					$cnt=$i+1;
					$ques_Add[$i]['question'] = $this->_request->getParam('question_'.$cnt);
					$ques_Add[$i]['answer'] = $this->_request->getParam('answer_'.$cnt);
					for($j=$i-1; $j>0; $j--){
						if($ques_Add[$i]['question'] == $ques_Add[$j]['question']){
							$a["MESSAGE"] = 'Same security question. Please choose different questions';
							$cthis->redirect('usererror','usererror','cw',$a);
						}
					}
				}

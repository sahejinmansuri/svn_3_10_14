...
class App_Event_Aw_ProfileController_edituserpassword extends App_Event_WsEventAbstract  {
		...
		$pview->userid = $uid;
            ...
            if ($this->_request->getParam('doaction') != null) {
                ...
                if (strlen($newpassword) >= 8) {
                        ...
                        $uinfof = $uinfo->update(
                                array(
                                        'password' => Atlasp_Utils::inst()->encryptPassword($newpassword)
                                ),
                                $uinfo->getAdapter()->quoteInto('user_id = ?', $item)
                        );

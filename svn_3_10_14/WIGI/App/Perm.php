...
const FUNDING_INDEX = 1;
const ORDERS_INDEX = 2;
...
public static function getCategories($c=0) {
	...
	$feature[self::FUNDING_INDEX                   ] = 'Funding'; 
	$feature[self::ORDERS_INDEX                   ] = 'Orders'; 
...
public static function getUserWigiFeatures($c = 0)
{
	$r=array(
		...
		'VIEW_DOCUMENTS_INDEX' => array(
			'index' => 2,
			'label' => 'My Document',
			'vname'  => 'VIEW_DOCUMENTS_INDEX',
			'class' => 's_dashboard',
		),
		'MESSAGES_INDEX' => array(
			'index' => 3,
			'label' => 'Messages',
			'vname'  => 'MESSAGES_INDEX',
			'class' => 's_dashboard',
		),
		'PREFERENCES_INDEX' => array(
			'index' => 4,
			'label' => 'Preferences',
			'vname'  => 'PREFERENCES_INDEX',
			'class' => 's_dashboard',
		),
		...
		'CHNAGE_PIN_INDEX' => array(
			'index' => 7,
			'label' => 'Change PIN',
			'vname'  => 'CHNAGE_PIN_INDEX',
			'class' => 's_dashboard',
		),
		'ADD_MONEY_INDEX' => array(
			'index' => 8,
			'label' => 'Add Money',
			'vname'  => 'ADD_MONEY_INDEX',
			'class' => 's_dashboard',
		),
		'WITHDRAW_MONEY_INDEX' => array(
			'index' => 9,
			'label' => 'Withdraw Money',
			'vname'  => 'WITHDRAW_MONEY_INDEX',
			'class' => 's_dashboard',
		),
		'MANAGE_MONEY_INDEX' => array(
			'index' => 10,
			'label' => 'Manage Money Sources',
			'vname'  => 'MANAGE_MONEY_INDEX',
			'class' => 's_dashboard',
		),
		'CHANGE_QUESTION_INDEX' => array(
			'index' => 11,
			'label' => 'Change Security Question',
			'vname'  => 'CHANGE_QUESTION_INDEX',
			'class' => 's_dashboard',
		),
		'LOCK_CELL_INDEX' => array(
			'index' => 12,
			'label' => 'Lock Cell Account',
			'vname'  => 'LOCK_CELL_INDEX',
			'class' => 's_dashboard',
		),
	);
...
public static function getCellphonePermission($c = 0)
{
	$r=array(
		'VIEW_PROFILE_INDEX' => array(
			'index' => 1,
...
public static function prepareUserSettingsUser($str)
{
	$resArr;
	$stra=explode("|",$str);
	//print_r($stra);

	$r=self::getUserWigiFeatures();
...
public static function prepareRolesDataUsers($a)
{
	$res=array();
	$features = self::getUserWigiFeatures();
	foreach($a as $id=>$data)
	{
		if(preg_match("/User_Roles_/",$id))
		{
			$rolename = str_replace("User_Roles_","",$id);
			$tmp['rec_id']=$rolename;
			$tmp['rolename']=str_replace("_"," ",$rolename);
			$tmp['value']=$data;
			$tmp['perms'] = self::prepareUserSettingsUser($data);
			$res[$rolename]=$tmp;
		}

...
class App_Transaction_Transaction  extends App_Models_Db_Wigilog_Transaction {
    ...
    if ($id > 0) {
      ...
      $this->res["Total Balance"] = $result->balance;
      $this->res["Available Balance"] = $result->temp_balance;
      $this->res["Type"] = $result->description;
      $this->res["IMPC"] = App_DataUtils::fmtCode($result->wigi_code_id);
  ...
  public static function log($type,$direction,$amount,$balance,$temp_balance,$from,$to,$description,$from_description,$to_description,
      ...
      $billing_amount = App_WigiEngine::billUser($from,$amount,$type);
      ...
      if (!array_key_exists("ip_address",$extinfo) ) {
        $extinfo["ip_address"] = "";
      }
      if (!array_key_exists("gps",$extinfo) ) {
        $extinfo["gps"] = "";
      }
      if (!array_key_exists("server_datetime",$extinfo) ) {
        $extinfo["server_datetime"] = "";
      }
      if (!array_key_exists("client_datetime",$extinfo) ) {
        $extinfo["client_datetime"] = "";
      }
      if (!array_key_exists("appname",$extinfo) ) {
        $extinfo["appname"] = "";
      }
      if (!array_key_exists("appversion",$extinfo) ) {
        $extinfo["appversion"] = "";
      }
      if (!array_key_exists("devicemodel",$extinfo) ) {
        $extinfo["devicemodel"] = "";
      }
      if (!array_key_exists("systemname",$extinfo) ) {
        $extinfo["systemname"] = "";
      }
      if (!array_key_exists("systemversion",$extinfo) ) {
        $extinfo["systemversion"] = "";
      }
      if (!array_key_exists("os",$extinfo) ) {
        $extinfo["os"] = "";
      }
      if (!array_key_exists("browser_string",$extinfo) ) {
        $extinfo["browser_string"] = "";
      }
      if (!array_key_exists("language",$extinfo) ) {
        $extinfo["language"] = "";
      }
      if (!array_key_exists("osid",$extinfo) ) {
        $extinfo["osid"] = "";
      }
      if (!array_key_exists("processor_transaction_id",$extinfo) ) {
        $extinfo["processor_transaction_id"] = "";
	  ...
	  if(!isset($to_user_id_description)){
		$to_user_id_description = "";
	  }
	  
	  if($direction == 'Info'){
		$settled = 'N/A';
	  }elseif(($type == '213') || ($type == '214') || ($type == '215') || ($type == '216')){
		$settled = 'No';
	  }else{
		$settled = 'Yes';
	  }
  ...
  public function getLastDaySendGiftAmtTotal($mobileid) {
      $db = new App_Models_Db_Wigilog_Transaction();
	  $select = $db->select();
	  $select->where("`from` = ?",$mobileid);
	  $select->where("`direction` = ?",'DEBIT'); 
	 
	  $raw = $db->fetchAll($select);
	  $total_amount = 0;
	  foreach( $raw as $key=>$val){
		$time_now = time();
		$date = date('d M Y', $time_now);
		$time_compare = strtotime($date);
		
		$time_gift = $val['stamp'];
		$timestamp = strtotime($time_gift);
		if($timestamp >= $time_compare){			
			$total_amount = $val['amount'] + $total_amount;
		}
	  }
	  
	  $select = $db->select();
	  $select->where("`from` = ?",$mobileid);
	  $select->where("`type` = ?",'200'); 
	 //error_log($select);
	  $raw = $db->fetchAll($select);
	  foreach( $raw as $key=>$val){
		$time_now = time();
		...
		$timestamp = strtotime($time_gift);
		if($timestamp >= $time_compare){
			$code = $val['wigi_code_id'];
			
			$select1 = $db->select();
			$select1->where("`from` = ?",$mobileid);
			$select1->where("`type` = ?",'201'); 
			$select1->where("`wigi_code_id` = ?",$code); 
			 //error_log($select1);
			$raw1 = $db->fetchAll($select1);
			$count_row = count($raw1);
			if($count_row == 0){
				$total_amount = $val['amount'] + $total_amount;
			}
		}
  ...
  public function getLastDayWigiAmt($mobileid) {
      $sp = new App_Db_Sp_GetLastDayAmt();
      $res = $sp->getSimpleResponse(array( 'MOBILEID'=>$mobileid , 'TYPE'=> App_Transaction_Type::REDEEMED_WIGI_CODE_CONSUMER )); //comment by attune and add new
	  //$res = $sp->getSimpleResponse(array( 'MOBILEID'=>$mobileid , 'TYPE'=> App_Transaction_Type::SEND_IMPC ));
      return $res['@p_res'];
  }
  ...
  public static function search($p,$timezone,$count=false) {
    ...
    if (array_key_exists("USER_MULTIPLE",$p)) {
      $s = "";
      foreach ($p["USER_MULTIPLE"] as $uid) {
          $s .= " `to_user_id` = '$uid' or";
      }
      preg_match('/^(.*) or$/', $s, $m);
      $select->where($m[1] . "?","");

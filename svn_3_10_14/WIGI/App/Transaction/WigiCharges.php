...
{
	public static function getWigiCharge($defFixed, $defPercentage, $defField, $userStr, $amt, $code,$specialField)
	{
		...
		$finalData = self::_mergeUserAndGlobalData($wData, $uData,$specialField);
		#print_r($finalData);
		#die();
	...
	public static function _mergeUserAndGlobalData($g, $u,$specialField)
	{
		//print_r($g);
		//print_r($u);
		$resArr=array();
		...
		/*if(!empty($specialField)){
			foreach($specialField as $key=>$val){
				$value = $val['value'];
				$special_billing = array();
				$value_array = explode('|',$value);
				foreach($value_array as $key1=>$val1){
					$final_value_array = explode('-',$val1);
					$key_put = $final_value_array[0];
					$special_billing[$key_put]['def'] = $final_value_array[1];
					$special_billing[$key_put]['val'] = $final_value_array[2];
				}
			}
		}*/
		...
		/*foreach($resArr as $key=>$val){
			if(array_key_exists($key, $special_billing)){
				$def = $special_billing[$key]['def'];
				if($def == 'N'){
					if($special_billing[$key]['val'] != 0){
						$val_special = $special_billing[$key]['val'];
						$val_res = $resArr[$key]['val'];
						$final_val_1 = ($val_res * $val_special)/100;
						$final_val = $val_res - $final_val_1;
						$resArr[$key]['val'] = $final_val;
					}
				}else if($def == 'Y'){
					$resArr[$key]['val'] = 0;
				}

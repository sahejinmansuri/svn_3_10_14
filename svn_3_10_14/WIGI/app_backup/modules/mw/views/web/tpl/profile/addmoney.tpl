			...
			<div class="information">
						...
						<form action="{$formbase}profile/addmoney" method="post" autocomplete="off">
							...
							<div class="stepbox">
								...
								<div class="prompt moneytype">
									...
									<select name="TYPE" id="type">{if $credit_cards < 0}<option value="CreditCard">Credit Card</option>{/if}{if $bank_accounts < 1}<option value="BankAccount">Bank Account</option>{/if}</select>
								...
								<div class="prompt iscard zip">
									<label for="zip">Zip code</label>
									<input type="text" name="ZIP" id="zip" maxlength="5" />
									<p class="tip">What is your zip code?</p>
								</div>
								<div class="prompt iscard address">
									<label for="address">Address 1</label>
									<input type="text" name="ADDRESS" id="address" maxlength="30" />
									<p class="tip">What is your address?</p>
								</div>
								<div class="prompt iscard address2">
									<label for="address">Address 2</label>
									<input type="text" name="ADDRESS2" id="address2" maxlength="30" />
									<p class="tip">Apartment, suite, etc.</p>
								</div>
								<div class="prompt iscard city">
									<label for="city">City</label>
									<input type="text" name="CITY" id="city" maxlength="24" />
									<p class="tip">Enter your city</p>
								</div>
								<div class="prompt iscard state">
									<label for="state">State</label>
									<select name="STATE" id="state"><option value=""></option>{foreach from=$states key=si item=st}<option value="{$st}">{$st}</option>{/foreach}</select>
				...
				{elseif $showcontent == "success"}
					...
					<div class="setup addedmoney">
						...
						<p>You now need to <a href="{$formbase}profile/confirmmoney/ITEM/{$ITEM}" style="text-decoration:underline;">confirm your account</a>. We have withdrawn two small random amounts from the bank account you provided (We will deposit it back!). You will need to check your bank account statement (online or paper) to see the amounts and confirm them. These withdrawls may take up to one day to appear in your bank statement.</p>
						{/if}

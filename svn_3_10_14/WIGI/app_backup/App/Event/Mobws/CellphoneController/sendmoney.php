...
class App_Event_Mobws_CellphoneController_sendmoney extends App_Event_WsEventAbstract {
    ...
    public function execute($type){
               ...
               // $c_to->checkConstraint($amount,'7',false);
                ...
                if ($c_to->isLocked()) {
                  $c_to->sendMessage("InCashMe&trade;: " . $u_from->getFirstName() . " has tried to send you ₹$amount, but your cellphone is currently locked. Please go to incashme.in for instructions.");
                ...
                if ($u_to->getStatus() === "unconfirmed") {
                  $c_to->sendMessage("InCashMe&trade;: " . $u_from->getFirstName() . " has tried to send you ₹$amount, but your email is not currently activated. Please check your email and click 'Verify Now' to active your account.");
                  throw new App_Exception_WsException('Cellphone is registered but unable to receive funds at this time. Please try again later.');
                  return false;
                }
                ...
                if (!($c_to->isActivated())) {
                  $c_to->sendMessage("InCashMe&trade;: " . $u_from->getFirstName() . " has tried to send you ₹$amount, but your cellphone is not currently active. Please go to incashme.in for instructions.");
                ...
                $c_from->sendMessage("InCashMe&trade;: You have sent ₹$amount to $to");
                $c_to->sendMessage("InCashMe: You have received ₹$amount from " . $u_from->getFirstName() . " at " . App_DataUtils::fmtphone($c_from->getCellphone()) . " Message: $message");

...
/*
    ...
    if (($type == 2) && ($this->prefs["wigi"]['max_per_trans'] <= $amount)  ) {
      throw new App_Exception_WsException('Can not spend more than ₹' . $this->prefs["wigi"]['max_per_trans']);
      return false;
    }
    ...
    if ($reduces == true && $sendmessage == true && $this->prefs["wigi"]["minbal"] > ($this->getTempBalance() - $amount)) {
      ...
      $this->sendMessage('Warning, account has gone below minumum balance of ₹' . $this->prefs["wigi"]["minbal"]);
    }

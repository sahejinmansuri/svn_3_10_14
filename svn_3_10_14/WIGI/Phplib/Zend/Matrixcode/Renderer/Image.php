...
class Zend_Matrixcode_Renderer_Image extends Zend_Matrixcode_Renderer_Abstract
{
	...
	protected function _renderMatrixcode()
	{
    	...
    	$new_color = $this->_matrixcode->getNewColor();
    	...
    	$symbolcolor1 = ImageColorAllocate($canvas, ($new_color & 0xFF0000) >> 16, ($new_color & 0x00FF00) >> 8, ($new_color & 0x0000FF));
		//echo $symbolcolor."=".$symbolcolor1;
    	if(!empty($back_color)) {
		...
		//print_r($matrix);
		//exit();
		$last = $matrix_dimension - 5;
		$value = array(2,3,4,$last,$last+1,$last+2);
		$value1 = array(2,3,4);
		$value2 = array($last,$last+1,$last+2);
		$border_first = array(0,1,2,3,4,5,6);
		$border_last = array($matrix_dimension,$matrix_dimension-1,$matrix_dimension-2,$matrix_dimension-3,$matrix_dimension-4,$matrix_dimension-5,$matrix_dimension-6);
		for($i=0; $i<$matrix_dimension; $i++) {
		    for($j=0; $j<$matrix_dimension; $j++) {
		    	if( $matrix[$i][$j] ) {
					...
					if(in_array($i,$value) && (in_array($j,$value))){
						if(in_array($i,$value2) && (in_array($j,$value2))){
							$flag = 0;
						}else{
							$flag = 1;
						}
					}
					else if(in_array($i,$border_first) && ($j == 0 || $j == 6 || $j == $matrix_dimension-1 || $j == ($matrix_dimension-7))){
						$flag = 1;
					}
					else if(in_array($j,$border_first) && ($i == 0 || $i == 6 || $i == $matrix_dimension-1 || $i == ($matrix_dimension-7))){
						$flag = 1;
					}
					else if(in_array($i,$border_last) && ($j == 0 || $j == 6 )){
						$flag = 1;
					}
					else if(in_array($j,$border_last) && ($i == 0 || $i == 6 )){
						$flag = 1;
					}
					else{
						$flag = 0;
					...
					if($flag == 0){
						...
						ImageSetPixel($canvas,$x,$y,$symbolcolor1);
					}
		        }
		    }
		}

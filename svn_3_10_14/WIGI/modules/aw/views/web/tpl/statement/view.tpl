	...
	<div id="content">
		...
		{foreach from=$statement key=k item=v}
			...
			<ul class="cellsummary">
				<li>Credit card funding <strong>₹{number_format({$v.cc_fund}, 2, '.', ',')}</strong></li>
				...
				<li class="clear">Total money sent (includes gifts) <strong>₹{number_format({$v.send_money}, 2, '.', ',')}</strong></li>
				<li>Total money received (includes gifts) <strong>₹{number_format({$v.recv_money}, 2, '.', ',')}</strong></li>
				<li>InCashMe&#8482; Scan and Buy <strong>₹{number_format({$v.scanandbuy}, 2, '.', ',')}</strong></li>
				<li>InCashMe&#8482; Scan and Pay <strong>₹{number_format({$v.scanandpay}, 2, '.', ',')}</strong></li>
				<li>Total WPC pending <strong>₹{number_format({$v.wigi_pending}, 2, '.', ',')}</strong></li>
				<li>Total WPC redeemed <strong>₹{number_format({$v.wigi_redeemed}, 2, '.', ',')}</strong></li>
			</ul>
				...
				<div class="dtable">
					<div class="dhead">
						<div class="drow">
							...
							<div class="dcol col6">Billing Charge</div>
						</div>
					</div>
					<div class="dbody">
						{foreach from=$v.transactions item=t}
							<div class="drow">
								...
								<div class="dcol col6">₹{number_format({$t.billing_amount}, 2, '.', ',')}</div>
							</div>
						{foreachelse}
							<div class="drow">

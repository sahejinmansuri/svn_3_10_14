	...
	<div id="content">
		...
		{foreach from=$statement key=k item=v}
			...
			<ul class="cellsummary">
				...
				<li>Payment disbursment <strong>US${number_format({$v.send_money}, 2, '.', ',')}</strong></li>
				<li>Vendor payments <strong>US${number_format({$v.send_money}, 2, '.', ',')}</strong></li>
				<li>Total cash received <strong>US${number_format({$v.recv_money}, 2, '.', ',')}</strong></li>
				<li>Total WPC redeemed <strong>US${number_format({$v.recv_money}, 2, '.', ',')}</strong></li>
				<li>Credit card swiped <strong>US${number_format({$v.recv_money}, 2, '.', ',')}</strong></li>
			</ul>

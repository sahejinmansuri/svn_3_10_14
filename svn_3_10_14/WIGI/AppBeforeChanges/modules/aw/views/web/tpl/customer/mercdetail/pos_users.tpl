            ...
            <div id="{$data.user_id}" style="display:none;padding:10px;background:#E4F5C8;padding:4px;">
                    ...
                    <td>
                       <div class="recrow ctr">
                        <a href="profile/edituser/ITEM/{$data.user_id}"><div class="adminBtn">Edit</div></a>
                        <a href="profile/deleteuser/ITEM/{$data.user_id}"><div class="adminBtn">Delete</div></a>
                        <a href="{$formbase}profile/edituserpassword/ITEM/{$data.user_id}"><div class="adminBtn">Change password</div></a>
                        {if $data.status == "active" or $data.status == "locked"}
                            <a href="{$formbase}profile/edituserstatus/ITEM/{$data.user_id}"><div class="adminBtn">Change Status</div></a>
                        {else}
                            <a href="#" onclick="$.wigialert('You can only change status for active and locked users.');return false;">Change status</a>
                        {/if}
                       </div>
                    </td>
                    </tr>

						...
						<form action="{$formbase}orders/scanandbuy" method="get">
							<ul class="filter">
								...
								<li>
									...
									<ul>
										<li><select name="SBM"><option value="">All</option>{foreach from=$merchantnames item=m}<option value="{$m}"{if $m == $SBM} selected="selected"{/if}>{$m}</option>{/foreach}</select></li>
									</ul>
								</li>
								<li>
								...
								{section name=loop loop=$orders['buy']['orders']}
									<div class="drow{if $smarty.section.loop.index%2} drowalt{/if}">
										<div class="dnormal">
											...
											<div class="dcol col5">US${number_format({$orders['buy']['orders'][loop].price}, 2, '.', ',')}</div>
										</div>
										<div class="dextend">
											...
											<div class="expandtype sbordersbox">
												<div>
													...
													<ul>
														...
														<li>Payment amount: US${$orders['buy']['orders'][loop].price}</li>
